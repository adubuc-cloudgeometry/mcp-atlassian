...
class UserTokenMiddleware:
    ...
    def _parse_auth_header(self, auth_header: str, scope: Scope) -> None:
        ...
        if auth_header.startswith("Bearer "):
            ...
            if not encoded:
                scope["state"]["auth_validation_error"] = (
                    "Unauthorized: Empty Basic credentials"
                )
            else:
                try:
                    ...
                    if ":" not in decoded:
                        scope["state"]["auth_validation_error"] = (
                            "Unauthorized: Basic credentials must be email:api_token"
                        )
                    else:
                        ...
                        logger.debug(
                            f"UserTokenMiddleware: Basic auth extracted for user: {email}"
                        )
                except Exception as e:
                    scope["state"]["auth_validation_error"] = (
                        f"Unauthorized: Invalid Basic credentials encoding: {e}"
                    )
            ...
            scope["state"]["auth_validation_error"] = (
                ...
                "'Token <PAT>', or 'Basic <base64(email:token)>' types are supported."
            )
        else:
